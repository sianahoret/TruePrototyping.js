...
* {Prototypal Inheritance in JavaScript"}[http://javascript.crockford.com/prototypal.html] by Douglas Crockford. Just a sentence from there: "JavaScript itself is conflicted about its prototypal nature".
...
Actually I am upset by most of popular javascript libraries and frameworks not only use the "traditional" style, but even enhance imitation of Classical OOP with class-like APIs, while I do not know a library which follows the true prototypal inheritance style. I guess why it is so (see Problems section below), but I assume the "true style" deserves to be more used than it is at nowadays.

I try to achieve two aims:
1. To make "true" style programming as possible compact and pleasing. I have mentioned ES5 standard intends a support of what I name the "true" style. As for me, the standard way is too verbose. So I propose some syntactical sugar.
2. To make lookup through true prototype chain easy and clear.

=== Library overview

There are two main features are provided by the library:

'derive' method :: Compact way for construction of object hierarchy with clear prototypal inheritance
'ancestor' and 'ancestors' properties, 'super', 'isAncestorOf' and 'isDescendantOf' methods :: Convenient way for introspection and lookup of true prototype chain, even if object hierarchy was created with "traditional" style.

=== Construction of object hierarchy with 'derive' method
...
Well, there is following benefit. Suppose having any object in program, you want to make a very similar object, almost the same, but slightly different. With the proposed approach you can derive new object from the existing one, not worrying about nature of the existing object (existing object may be a Function or not, may have or not proper "prototype" or "constructor" properties):
...
=== Use for introspection of object hierarchies have been build with different frameworks

Frameworks which provides APIs for constructing object hierarchy (most of them simulate classical inheritance) in different ways provides the feature of referencing to ancestor object (up through the hierarchy), well known as "super"-call. Sometimes the feature is provided explicitly, sometimes it is there, but is not documented (and looks like inner __super__ method), sometimes it is just absent.

TruePrototyping library provides a few properties and methods, which allow to walk through true prototype chain: 'ancestor' and 'ancestors' properties, 'super', 'isAncestorOf' and 'isDescendantOf'. Moreover, they allow to do that in the same way for object hierarchies created by different frameworks. In the mix_with_frameworks directory there are examples of using the mentioned way for object hierarchies created with Propotype, Mootools, Backbone and TruePrototyping - you can clone the repository, open mix_with_frameworks/all.html file in browser and try that.
...
* Maybe it makes sense to provide a mechanism of preventing conflicts (what, if some other library also has 'ancestor' property?).
