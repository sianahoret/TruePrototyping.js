...
TruePrototyping library provides a few properties and methods, which allow to walk through true prototype chain in the same way for different frameworks. In the mix_with_frameworks directory there are examples of using the mentioned way for object hierarchies created with Propotype, Mootools, Backbone and TruePrototyping - you can clone the repository, open mix_with_frameworks/all.html file in browser and try that.
