...
The library tries to achieve the following aims:
1. To make "true" prototypal programming in ES5-compliant environment as possible compact and pleasing. As it is mentioned above, ES5 standard intends a support of the style, but, as for me, the way is too verbose. 
2. To provide ability to use the approach in non-ES5-compliant environment with the same syntax.
3. To make lookup through true prototype chain easy and clear (regardless to a way the chain was created).

So the library provides the following groups of properties and methods:
...
If you try to do this:
  ...
  MyModel = Backbone.Model.extend({});
  myInstance = MyModel.derive();        // myInstance.__proto__ points to MyModel itself instead of MyModel.prototype
...
=== Use for lookup in object hierarchies have been build with different frameworks
...
TruePrototyping library provides a few properties and methods, which allow to walk through true prototype chain: 'ancestor' and 'ancestors' properties, 'super', 'isAncestorOf' and 'isDescendantOf' methods. Moreover, it allows to do that in the same way for object hierarchies created by different frameworks. In the mix_with_frameworks directory there are examples of using the mentioned way for object hierarchies created with Propotype, Mootools, JavascriptMVC, Backbone and TruePrototyping - you can clone the repository, open mix_with_frameworks/all.html file in browser and try that.

=== TODO

* Implementation for an environment which is not ES5-compliant. For now I am not sure it is actually needed.
