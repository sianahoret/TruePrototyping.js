...
* Make tpAncestorHaving(propertyName) and tpAncestorHavingDifferent(propertyName) methods which returns an ancestor having the specified property. These methods can be useful, when you need to find an ancestor by the logic like tpSuper/tpSuperImmediate currently have, but unlike in case of calling tpSuper/tpSuperImmediate you need to execute the specified property/method on its owner, not on 'this'. These methods, unlike tpSuper/tpSuperImmediate, can accept either method (function) or property (not function) name. The only problem is that the logic of tpAncestorHavingDifferent is unclear for property (not fuction), perhaps for such property tpAncestorHaving and ttpAncestorHavingDifferent will work equally :(.
