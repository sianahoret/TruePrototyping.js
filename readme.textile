...
* *The library does not create a wrapper over a target object* (as jQuery does, for instance). All the added properties and methods can be called on the target object immediately. It allows a programmer to be free from keeping in mind more abstractions than he needs, a programmer never should to skip from wrapper to underlying object and vice versa, because a wrapper does not exist :).
* *The library does not patch every concrete object by demand to apply a new behavior to it* (as Prototype does, for instance). Instead Object.prototype is patched and that's all. There are also a few things which are lazily applied to an object, when such necessity appears. It allows a programmer to be free from extra actions (preprocessing of an object) than he needs.
...
<pre>
// This is very honest example as it is a part of TruePrototyping library definition, you can find it inside the library source.
// I just wanted to have an object, which behaves like instance of standard Array, but has 'last' method.

MyArray = Array.prototype.tpDerive({                // here you can write also [].tpDerive to to be even more abstracted from 'prototype' property, just an extra [] object will be in the MyArray's prototype chain it such case
  last: function(obj){
    return this.length > 0 ? this[this.length-1] : null;
  }
});

// Since now MyArray can be used as an ancestor ("type") of as many objects ("instances") as you want.

theStack = MyArray.tpDerive();
